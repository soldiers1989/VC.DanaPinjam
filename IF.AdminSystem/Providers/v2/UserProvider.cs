...
namespace NF.AdminSystem.Providers.v2
{
    public class UserProvider
    {
        ...
        public static DataProviderResultModel SaveUserBankInfo(UserBankInfoModel bankInfo)
        {
            DataBaseOperator dbo = null;
            DataProviderResultModel result = new DataProviderResultModel();
            try
            {
                ...
                Hashtable table = new Hashtable();
                DataTable dt = dbo.ExecProcedure("p_user_editbankinfo_v3", pc.GetParams(), out table);
                if (null != dt && dt.Rows.Count == 1)
                {
                    int.TryParse(Convert.ToString(dt.Rows[0][0]), out result.result);
