    ...
    public class UserController : ControllerBase
    {
        ...
        public ActionResult<string> UserLogin(string phone, string password, string code = "", int loginType = 0)
        {
            ...
            try
            {
                ...
                if (result.result > 0)
                {
                    ...
                    string key = String.Format("UserAllInfo_{0}", userInfo.userId);
                    redis.KeyDelete(key);
                }
                else
                {
        ...
        public ActionResult<string> EditUserWorkingInfo([FromQuery]UserWorkingInfoModel workingInfo)
        {
            ...
            try
            {
                ...
                {
                    Redis redis = HelperProvider.GetRedis();
                    string key = String.Format("UserAllInfo_{0}", workingInfo.userId);
                    redis.KeyDelete(key);
                }
            }
            catch (Exception ex)
        ...
        public ActionResult<string> EditUserContactInfo([FromQuery]UserContactInfoModel contactInfo)
        {
            ...
            try
            {
                ...
                if (result.result == Result.SUCCESS)
                {
                    ...
                    Redis redis = HelperProvider.GetRedis();
                    string key = String.Format("UserAllInfo_{0}", contactInfo.userId);
                    redis.KeyDelete(key);
                }
                else
                {
        ...
        public ActionResult<string> PostUserLocation()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    Redis redis = HelperProvider.GetRedis();
                    string key = String.Format("UserAllInfo_{0}", location.userId);
                    redis.KeyDelete(key);
